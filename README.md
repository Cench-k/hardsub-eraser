...
| 구성 | 감지 | 인페인팅 | PSNR | SSIM |
|---|---|---|---|---|
| 청크 방식 (초기) | — | 2.6 fps | — | — |
| group=5 refs=8 stride=1 | 12.7s | 3.7 fps | 23.78 dB | 0.8673 |
| **group=9 refs=8 stride=3 (자동)** | **5.5s** | **4.6 fps** | **23.90 dB** | **0.8693** |

**group_size 를 키우는 게 공짜 속도다.** 트랜스포머는 t = group_size + n_refs 프레임을
처리해 group_size 개를 내놓는다. 참조 수를 그대로 둔 채 group_size 만 키우면 같은
비용이 더 많은 출력에 분산된다. 참조가 그대로라 품질도 그대로다(오히려 미세하게 올랐다).

GTX 1060 3GB 실측:

| group | refs | t | s/frame | 배수 |
|---|---|---|---|---|
| 5 | 8 | 13 | 0.251 | 1.00x |
| 8 | 8 | 16 | 0.190 | 1.32x |
| **10** | 8 | 18 | **0.177** | **1.41x** |
| 12 | 8 | 20 | 0.447 | 0.56x ← 절벽 |

t=20 을 넘는 순간 공유 메모리로 밀려 2.5배 느려진다. 최적점이 GPU마다 다르고
이렇게 급격히 꺾이므로 계산으로는 못 맞춘다. autotune 이 실제로 재면서 찾는다.
...
이름도 무작위라 **결과물을 못 찾는 일이 실제로 있었다.** Windows 디스크 정리가
언제든 비우기도 한다.

`지난 작업` 목록은 이 폴더를 직접 읽는다. 작업 폴더(임시)에서 읽으면 임시 파일을
정리하는 순간 이력이 통째로 사라진다.

## 로컬 서버 보안

`127.0.0.1` 로만 listen 한다고 안전하지 않다. 사용자가 아무 웹사이트나 열어두면
그 페이지의 스크립트가 로컬 서버로 요청을 보낼 수 있다. 초기 버전은 CORS 가
`allow_origins=["*"]` 라 응답까지 읽혔고, 작업 생성이 로컬 임의 경로를 받으므로
외부 페이지가 이 PC의 영상을 지목해 받아갈 수 있었다.

- CORS 미들웨어 제거 (같은 출처에서만 쓰므로 불필요)
- `Origin` 검사 — 다른 출처의 요청을 403. 프리플라이트 없는 단순 POST 도 걸린다
- `Host` 검사 — DNS 리바인딩 방어
...
- 같은 자막이 오래 머무르고 배경이 움직이면 옅은 얼룩이 남는다. 참조 구간을 넓혀
  많이 줄였지만 완전히 없애진 못했다.
- **PSNR 23.9 dB 는 절대 수치로는 높지 않다.** 인페인팅은 '그럴듯하지만 원본과 다른'
  결과에 PSNR 이 가혹하므로 상대 비교용으로만 쓰는 게 맞다.
- 처리 시간은 실시간의 약 6배. 1분 30fps 영상에 약 6분.
